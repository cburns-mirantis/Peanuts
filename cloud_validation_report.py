...
parser.add_argument('-fu', '--fuel-user', action='store', dest='fuel_username', type=str, help='Fuel Web Username',default='admin')
parser.add_argument('-fp', '--fuel-pw', action='store', dest='fuel_password', type=str, help='Fuel Web Password',default='admin')
...
parser.add_argument('-fuelp', '--fuel-port', action='store', dest='fuel_port', type=str, help='Fuel Web Port',default='8443')
args = parser.parse_args()

def get_token():
    header = {'Content-Type': 'application/json', 'accept': 'application/json'}
    creds = {'auth': {'tenantName': args.fuel_username,'passwordCredentials': {'username': args.fuel_username,'password': args.fuel_password}}}
    try:
        r = requests.post(url='https://' + args.host + ':' + args.fuel_port + '/keystone/v2.0/tokens',headers=header,verify=False,json=creds,timeout=15)
    except:
...
def get_nodes(token):
    ...
    return sorted(json.loads(requests.get(url='https://' + args.host + ':' + args.fuel_port + '/api/nodes',headers=header, verify=False).text), key=lambda k: k['hostname'])

def get_clusters(token):
    header = {'X-Auth-Token': token,'Content-Type': 'application/json'}
    return json.loads(requests.get(url='https://' + args.host + ':' + args.fuel_port + '/api/clusters',headers=header, verify=False).text)

def get_network(token,cluster_id):
    header = {'X-Auth-Token': token,'Content-Type': 'application/json'}
    return json.loads(requests.get(url='https://' + args.host + ':' + args.fuel_port + '/api/clusters/' + str(cluster_id) + '/network_configuration/neutron/', headers=header, verify=False).text)
...
def launch_5_vms_and_verify():
    try:
        ...
        print("Created keypair",keypair.name)
    ...
    for i,s in enumerate(range(2)):
        try:
            ...
            print("Created floating ip",floating_ips[-1].ip)
            print("Created instance",servers[-1].name)
        except (requests.exceptions.ConnectionError, requests.exceptions.ChunkedEncodingError, BadRequest,ClientException) as e:
            print(e)
            print("test failed")
            return False
    ...
    for i,s in enumerate(servers):
        if "HA Test-" in s.name:
            try:
                print("Attaching",floating_ips[i].ip,"to",s.name)
    ...
    for f in floating_ips:
        while(socket.socket(socket.AF_INET, socket.SOCK_STREAM).connect_ex((f.ip, 22)) is not 0):
            print("Port 22 not available for", f.ip,"yet, waiting 5 seconds...")
            time.sleep(5)
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(f.ip, username="cirros", password="cubswin:)")
        if ssh.get_transport().is_active():
            print("Successfully created ssh session with",f.ip)

    print("Destroying keypair & security group")
...
def perform_ha_tests(id):
    ...
    # ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('ssh root@' + node_ip + ' shutdown -h 0\'')


    # input("Press enter once the host has been gracefully shutdown.")
    # while(True):
    #     if os.system('ping -c 1 -t 5 ' + node_ip) == 0:
    #         print("Waiting 5 seconds for host to become unavailable...")
    #         time.sleep(5)
    #         continue
    #     else:
    #         print("Host is down.")
    ...
    input("Press enter once the controller has booted.")
    while(True):
        if os.system('ping -c 1 -t 5 ' + node_ip) != 0:
            print("Waiting 5 seconds for host to become available...")
            time.sleep(5)
            continue
        else:
            print("Host is up. Verifying if haproxy services have settled...")
            print('ssh root@' + test_ip + ' /usr/bin/haproxy-status.sh | grep DOWN | wc -l')
            ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('ssh root@' + test_ip + ' /usr/bin/haproxy-status.sh | grep DOWN | wc -l')
            result = ssh_stdout.readlines().pop().replace("\n","")
            print(result)
            # print(ssh_stdout.readlines().pop().replace("\n",""))
            while(result is not "0"):
                print(result)
                print("haproxy services still settling, waiting 5 seconds...")
                time.sleep(5)
                ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('ssh root@' + test_ip + ' /usr/bin/haproxy-status.sh | grep DOWN | wc -l')
                result = ssh_stdout.readlines().pop().replace("\n","")
            break
    ...
    launch_5_vms_and_verify()
    # ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('ssh root@' + node_ip + ' \"echo c > /proc/sysrq-trigger\"\'')
    ...
    while(True):
        if os.system('ping -c 1 -t 5 ' + node_ip) != 0:
            ...
            break
    launch_5_vms_and_verify()
    # ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('ssh root@' + node_ip + ' ip link set dev br-mgmt down;sleep 120;ip link set dev br-mgmt up\'')
